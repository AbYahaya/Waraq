...
| ✅ | kraken (eScriptorium out of scope per project owner) | Manuscripts/calligraphy | *2026-05-10 (Phase 4 sub-batch kraken): `waraq/ocr/kraken.py` ships the adapter — `extract_with_confidence(image_bytes, mime_type) → KrakenResult(text, confidence)`, mirroring the Cloud Vision shape. Lazy imports (`kraken`, `PIL`) keep hosts without the package importable. `KRAKEN_MODEL_PATH` env var picks the recognition model (defaults to `arabic_best.mlmodel` in cwd). `is_available()` reflects package + model presence. `KrakenUnavailable` for install/model gaps (canon-honest no-signal — consensus driver's `_safe` wrapper converts to `error_class` verdict); `KrakenRecognitionError` for SDK exceptions (F-XX-routable). eScriptorium (Django frontend) deliberately out of scope.* |
...
| ✅ | kraken for manuscripts/calligraphy (gate behind project-flag) | §3.3 — *2026-05-10 (Phase 4 sub-batch kraken): `OcrEngine.KRAKEN` added to the canonical engine enum. Routing layer `engines_for(block_class, *, use_kraken=False)` adds KRAKEN to MAIN_TEXT/HEADING/FOOTNOTE/HADITH/MARGINALIA when the flag is set; QURAN stays Gemini-only (Qurʾān script is printed-only canonically). Consensus driver `run_engines(..., kraken_fn=None, use_kraken=False)` invokes kraken when flag + callable both present; partial wiring degrades gracefully to 2-engine path. The "project-flag" is materialized as the `use_kraken` function-call boundary parameter — no DB column added in this sub-batch (CLAUDE.md §2.3: canon names the gate but not its schema, so we do not invent one). When a future canon-amendment specifies schema (`Project.ocr_use_kraken`), the column plumbs into this kwarg without changing routing-table semantics. eScriptorium deliberately out of scope per project owner. Diagnostics surface `POST /diagnostics/kraken/recognize` (multipart image upload) lets the user verify kraken on a manuscript scan without first wiring the flag into the workspace UI. 18 new tests (1 skipped — kraken package not installed in the venv; that branch IS what `is_available() == False` returns, and the adapter raises `KrakenUnavailable` for it).* |
...
### kraken manuscript-OCR — useful-corpus question (parked 2026-05-11)

**State today.** Phase 4 sub-batch kraken landed the adapter + routing gate + diagnostics surface (CANON_TRACKER row 94 + 352 ✅). The adapter is wired, kraken 7.0.2 is installed in `backend/.venv` (~5.2 GB overhead), and a printed-Arabic-script model is downloaded and reachable via `KRAKEN_MODEL_PATH`. End-to-end smoke test on the project's Al-Fatiha fixture succeeded (27 s CPU, 0.885 mean confidence).

**The gap.** Research on 2026-05-11 confirmed **no publicly available pretrained kraken model targets classical Islamic manuscripts** (Fiqh, Hadith, naskh / Maghribi / Andalusi / Mamluk-era). Candidates examined: Muharaf (19th–21st c. Christian Arabic correspondence — wrong era/genre), TariMa/RASAM (modern Maghrebi, ~85% — closest paleographic family to Andalusi but accuracy too low for production), OpenITI `ms_*` (undocumented), Agapet (dataset only, no model). What we have today: printed-Arabic model — duplicates Gemini + Cloud Vision capability without adding manuscript coverage.

**Why this is parked, not closed.** The canon row says "kraken + eScriptorium for manuscripts/calligraphy". Removing eScriptorium from scope (user decision, 2026-05-10) leaves the kraken row's *intent* canon-deferred even though its *adapter* is canon-faithful. A real choice between four options is needed before the work hardens into either direction.

**Option A — Accept the gap, uninstall kraken from `.venv`.** Adapter code stays (canon row stays ✅, "wired but inactive" state). Frees ~5.2 GB. Diagnostics pill goes red. Re-activation later is one `pip install kraken && kraken get …` away — zero code changes. Best fit if daily corpus is overwhelmingly printed editions.

**Option B — Train a custom model without eScriptorium.** Public datasets exist in kraken-compatible PAGE-XML / JSON: RASAM (11,290 Maghrebi lines, `calfa-co/rasam-dataset`), TariMa (additional Maghrebi), BADAM (Arabic-script baselines), Agapet (3 medieval Christian Arabic manuscripts). Recipe: start from `model-manuscript_best.mlmodel` (Maghrebi seed, 85%), then `ketos train --resume` on a few hundred lines of user-transcribed Fiqh/Hadith ground truth. Bottleneck is human transcription time, not infrastructure. Annotation tools: direct PAGE-XML editing, Aletheia, Transkribus — all eScriptorium-free.

**Option C — Add eScriptorium back into scope.** Standard canon path. Multi-week build to host eScriptorium + integrate it + use it to produce training corpus. CR-shaped reversal of the 2026-05-10 scope decision.

**Option D — Skip kraken entirely as v1.0 work; revisit in v2.0.** Honest framing: classical-Arabic manuscript OCR is genuinely thin in the open-source world as of 2026-05-11. Even with eScriptorium, the user would still be producing transcribed ground truth from scratch. Re-evaluate in 6–12 months: (a) better pretrained models may emerge publicly, (b) Gemini-Vision's manuscript performance is improving and may close the gap, (c) accumulated user manuscript material may justify the training investment.

**Decision needed.** No deadline; the wired-but-inactive state is stable. Surface again when the user starts hitting manuscripts that Gemini + Cloud Vision can't handle, OR when v2.0 planning begins.
